(ns scicloj.reel.action-test
  (:require [clojure.test :refer [deftest testing is are]]
            [scicloj.reel.core :as reel]
            [scicloj.reel.protocols :as p]
            [scicloj.reel.impl.dihedral :as dih]
            [scicloj.reel.impl.product :as prod]
            [scicloj.reel.impl.cyclic :as cyc]
    ...
    (let [G (dih/dihedral-group n)
      ...
      (testing (str "row orthogonality for D_" n)
        (doseq [i (range num-irreps)
                j (range num-irreps)]
          (let [ip (c/abs (ch/character-inner-product (table i) (table j) sizes order))]
            (if (= i j)
    ...
    (doseq [n [3 4 5 6 8]]
      (let [G (dih/dihedral-group n)
            ...
            sum-sq (reduce + (map (fn [row]
                                    (let [d (long (Math/round (.x ^fastmath.vector.Vec2 (row 0))))]
                                      (* d d)))
                                  table))]
        (is (= order sum-sq)
            (str "Σ d² ≠ 2n for D_" n))))))
