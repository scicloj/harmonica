(ns
 reel-book.representation-matrices-generated-test
 (:require
...
(def v3_l25 (def ir-21 (reel/irrep [2 1])))


(def v4_l27 (reel/rep-dimension ir-21))


(deftest t5_l29 (is (= v4_l27 2)))


(def v6_l31 (def ir-31 (reel/irrep [3 1])))


(def v7_l33 (reel/rep-dimension ir-31))
...
(def
 ...
 (let
  [results
   (for
    [n (range 2 8) lambda (reel/partitions n)]
    (=
...
(def
 ...
 (let
  [results
   (for
    [n [3 4 5 6] lambda (reel/partitions n)]
    (let
     [ir
      (reel/irrep lambda)
      d
      ...
      (identity-matrix d)
      rho-e
      (reel/rep-matrix ir (reel/identity-perm n))
      diff
      (fm/sub rho-e I)
      err
      (Math/sqrt (fm/trace (fm/mulm diff (fm/transpose diff))))]
 ...
 (let
  [results
   (for
    [n [3 4 5] lambda (reel/partitions n)]
    (let
     [G (reel/symmetric-group n) ir (reel/irrep lambda)]
     (every?
      (fn
       [sigma]
       (let
        [rho-inv
         (reel/rep-matrix ir (reel/inv G sigma))
         rho-t
         (fm/transpose (reel/rep-matrix ir sigma))
         diff
...
(def
 ...
 (let
  [results
   (for
    [n [3 4 5] l1 (reel/partitions n) l2 (reel/partitions n)]
    (let
     [ir1
      (reel/irrep l1)
      ir2
...
(def
 ...
 (let
  [results
   (for
    [n [3 4] l1 (reel/partitions n) l2 (reel/partitions n)]
    (let
     [G
      (reel/symmetric-group n)
      ir1
   ...
   (for
    [n
     ...
     (take
      5
      (for
       [a (reel/partitions n) b (reel/partitions n) :when (not= a b)]
       [a b]))]
    (let
     [G
      (reel/symmetric-group n)
    ...
    (for
     [a
      ...
      [ira
       ...
       (irreps b)
       da
       (reel/rep-dimension ira)
       db
       (reel/rep-dimension irb)]
      i
      (range da)
      j
      (range da)
        ...
        (reduce
         ...
         (map
          (fn
           ...
           (let
            [Ma
             (reel/rep-matrix ira sigma)
             Mb
             (reel/rep-matrix irb sigma)]
   ...
   (every?
    ...
    (for
     [a
      (range (count irreps))
      :let
      [ira (irreps a) da (reel/rep-dimension ira)]
      i
      (range da)
      j
      (range da)
