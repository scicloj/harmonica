...
Tests include:
- Hand-written tests in `test/scicloj/reel/`
- Generated tests from notebooks (via `kind/test-last`) in `test/reel_book/`
...
- `test/scicloj/reel/` — hand-written tests
- `test/reel_book/*_generated_test.clj` — generated from notebooks via Clay GFM rendering

### Generated Test Workflow

Notebooks use `kind/test-last` to embed assertions. When rendered to GFM via Clay,
test files are generated in `test/reel_book/`. These are run by the standard test runner.

To regenerate tests after editing a notebook:
```clojure
(require '[dev :as dev] :reload)
(dev/make-gfm! "reel_book/<name>.clj")
```
...
- **Use `def ^:private` not `defn-` in notebooks**: `defn-` displays its var name (`#'reel_book...`) in rendered output. Use `(def ^:private name (fn [...] ...))` to suppress this noise.
