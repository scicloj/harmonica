  ...
  (kind/table
   {:column-names ["$\\lambda$" "$d_\\lambda$" "$d_\\lambda^2$"]
    :row-vectors (conj (mapv (fn [lam]
                               (let [d (reel/hook-length-dimension lam)]
                                 [(str lam) d (* d d)]))
                             parts)
                       ["**Total**" ""
                        (reduce + (map #(let [d (reel/hook-length-dimension %)]
                                          (* d d))
                                       parts))])}))
...
(let [n 5
      ...
      uniform (/ 1.0 n-elts)
      tv-data (mapv (fn [k]
                      (let [probs (reel/gsr-distribution-vec elts k)
...
(let [tv-data (vec
               (for [n [4 5 6]
                 ...
                 (let [G (reel/symmetric-group n)
                       ...
                       n-elts (count elts)
                       uniform (/ 1.0 n-elts)
                       probs (reel/gsr-distribution-vec elts k)
                       tv (* 0.5 (reduce + (map #(Math/abs (- (aget ^doubles probs (int %)) uniform))
                                                (range n-elts))))]
                   {:k k :tv tv :n (str "n=" n)})))]
  (-> (tc/dataset tv-data)
